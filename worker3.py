...
# Enable the debugger
# tf.debugging.experimental.enable_dump_debug_info("/Volumes/ssdmac/ray/tf_debug/worker2")
...
# tf.debugging.experimental.enable_dump_debug_info("/Volumes/ssdmac/ray/3", tensor_debug_mode="FULL_HEALTH", circular_buffer_size=-1)
...
class Worker(gym.Env):
    def __init__(self,  initial_capital=1e6, allocation_threshold=0.05, trading_cost=0.001, initial_shares_held=0,
        ...
        # To store the net position after each trade
        self.stock_holding_memory = np.array(
            [initial_shares_held], dtype=np.float32)
        # To store the net position after each trade
        self.position_memory = np.array([0], dtype=np.float32)
        # Debugging info
    ...
    def step(self, action, controller_action):
        ...
        if done:
            ...
            assert np.isfinite(
                end_adj_portfolio_value), f"end_adj_portfolio_value is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
            # print("end_adj_portfolio_value", self._calculate_assets()/1000000)
            portfolio_return = np.float32(0) if begin_adj_portfolio_value == 0 else np.float32(
                (end_adj_portfolio_value / begin_adj_portfolio_value) - 1)
            assert np.isfinite(
                portfolio_return), f"portfolio_return is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
            ...
            assert np.isfinite(
                total_pnl), f"total_pnl is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
            self.total_pnl_history.append(total_pnl)
            self.reward = self._calculate_reward(
                begin_adj_portfolio_value, end_adj_portfolio_value)
            assert np.isfinite(
                self.reward), f"self.reward  is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
            self.trading_pentalty = 0
            self.current_step_cost = 0
        truncated = False
        info = {}
    ...
    def _calculate_reward(self, begin_adj_portfolio_value, end_adj_portfolio_value):
        ...
        assert np.isfinite(
            pnl), f"pnl  is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
    ...
    def _handle_trading(self, action):
        ...
        if action < 0:  # Selling
            ...
            sell_amount = self.current_price * \
                shares_to_sell * (1 - self.trading_cost)
            assert np.isfinite(
                sell_amount), f"sell_amount  is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
            ...
            if self.current_cash <= 0.9:
                ...
                self.invalid_action_count += 1
                return 0
            max_affordable_shares = self.current_cash / \
            ...
            assert np.isfinite(
                buy_amount), f"buy_amount  is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
        ...
        assert np.isfinite(
            net_position), f"net_position  is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
        self.position_memory.append(net_position)
        self.shares_held += shares_to_buy - shares_to_sell
        assert np.isfinite(
            self.shares_held), f"self.shares_held   is not finite (could be NaN, inf, or -inf) at day {self.day} and worker {self.worker_id}"
        self.current_step_cost += self.current_price * \
            (abs(shares_to_sell) + abs(shares_to_buy)) * self.trading_cost
    ...
    def _calculate_pnl(self):
        ...
        pnl = (self.cash_from_sales - self.cash_spent) + \
            self.current_price * self.shares_held
    ...
    def _get_state(self):
        ...
        tech_indicators = [np.float32(stock_data[tech])
        ...
        state = {
            ...
            # Total trading costs
            'total_costs': np.array([self.total_costs], dtype=np.float32),
            # Current step in the episode
            'day': np.array([self.day], dtype=np.float32),
    ...
    def _reset_to_initial_values(self):
        ...
        self.trading_memory = [0]
        # To store the net position after each trade
        self.stock_holding_memory = [self.initial_shares_held]
        # To store the net position after each trade
